...
const TAB_KEYS = ["list", "form", "stats", "settings"];
const TAB_DEFAULT_LABELS = { list: "訂單", form: "新增", stats: "統計", settings: "設定" };
const DEFAULT_EMOJI_SETTINGS = {
  titleEmoji: "",
  tabEmojis: { list: "", form: "", stats: "", settings: "" },
};
...
function cropDataUrlByBBox(dataUrl, bbox, maxDim = 220, quality = 0.6) {
  return new Promise((resolve, reject) => {
    const img = new window.Image();
    img.onload = () => {
      ...
      if (sw < 4 || sh < 4) { reject(new Error("bbox 太小")); return; }
      let outW = sw, outH = sh;
...
function normalizeParsedResult(parsed) {
  let bbox = null;
  const rawBBox = parsed.thumbnail_bbox;
  if (rawBBox && typeof rawBBox === "object") {
    const { x, y, width, height } = rawBBox;
    const nums = [x, y, width, height];
    if (nums.every((v) => typeof v === "number" && isFinite(v)) && width > 0.03 && height > 0.03 && x >= 0 && y >= 0 && x + width <= 1.05 && y + height <= 1.05) {
      bbox = { x: Math.max(0, x), y: Math.max(0, y), width, height };
    }
  }
  return {
...
const RECOGNITION_PROMPT =
  "這是一張網購訂單截圖裡裁切出來的局部區域，通常包含一項商品的資訊：商品縮圖、商品名稱、規格（顏色/尺寸等）、數量、單價（有時還有一個被劃掉的原價）。\n" +
  "如果這個範圍裡不小心包含了不只一項商品，請只判讀畫面中『最完整、最上面那一項』商品，忽略其他項目。\n" +
  "請只回傳一個 JSON 物件，不要有任何其他文字、說明、Markdown 符號或程式碼區塊標記。JSON 格式固定為：\n" +
  '{"name": "商品名稱字串", "spec": "規格字串", "quantity": 數量數字, "price": 單價數字, "thumbnail_bbox": {"x": 數字, "y": 數字, "width": 數字, "height": 數字}}\n\n' +
  "各欄位規則：\n" +
  ...
  "- thumbnail_bbox：這個裁切區域裡『該商品的小張商品縮圖』相對於整張圖片的位置與大小，數值都是 0 到 1 之間的小數，(0,0) 是左上角、(1,1) 是右下角，x/y 是縮圖左上角位置，width/height 是縮圖佔整張圖的寬高比例。" +
  "如果畫面中有好幾張圖片（例如上方有一張很大、很模糊的宣傳背景圖或人物圖，下方或旁邊才是小小一張正方形的商品縮圖），請務必框選『那張小的商品縮圖』，不要選大張的宣傳圖、背景圖或人像圖。" +
  "如果完全找不到任何商品縮圖，thumbnail_bbox 請填 null。";
...
function guessFieldsFromOcrText(text) {
  ...
  return { name, spec, quantity, price, thumbnailBBox: null };
}
...
function blankOrder() {
  return {
    ...
    shipping: "",
    shippingMethod: "",
    payment: { method: "信用卡", bank: "", cardName: "", otherLabel: "" },
    rewards: [],
  };
}
...
function App() {
  ...
  function startEdit(order) {
    setDraft(JSON.parse(JSON.stringify({
      images: [], ...order,
      payment: { method: "信用卡", bank: "", cardName: "", otherLabel: "", ...order.payment },
  ...
  function exportBackup() {
    const payload = {
      orders, platforms, categories, shippingMethods, banks, cardNames, themeId, customThemes, fontId, customFontName,
  ...
  const statsOrders = useMemo(() => {
    ...
    return orders.filter((o) => {
      ...
      if (statsMode === "year") return d.getFullYear() === y;
  ...
  const stats = useMemo(() => {
    ...
    statsOrders.forEach((o) => {
      const c = calcOrder(o);
      paidSum += c.finalPaid;
      discountSum += c.totalDiscount;
      shippingSum += c.shipping;
      rewardSum += c.totalReward + c.couponRewardTokenAmount;
      costSum += c.actualCost;
      byPlatform[o.platform] = (byPlatform[o.platform] || 0) + c.finalPaid;
      (o.items || []).forEach((i) => {
        const key = i.category || "其他";
        byCategory[key] = (byCategory[key] || 0) + (Number(i.price) || 0) * (Number(i.qty) || 0);
      });
      c.rewards.forEach((r) => {
        const key = r.type === "自訂" ? r.label || "自訂回饋" : r.type;
        byReward[key] = (byReward[key] || 0) + r.amount;
      });
      if (c.shopTokenAmount > 0) {
        const key = (o.shopCoupon && o.shopCoupon.label) || "賣場回饋代幣";
        byReward[key] = (byReward[key] || 0) + c.shopTokenAmount;
      }
      if (c.platformTokenAmount > 0) {
        const key = (o.platformCoupon && o.platformCoupon.label) || "平台回饋代幣";
        byReward[key] = (byReward[key] || 0) + c.platformTokenAmount;
      }
      if (o.date) {
        ...
        byMonth[monthKey] = (byMonth[monthKey] || 0) + c.finalPaid;
      }
      const method = (o.payment && o.payment.method) || "其他";
      if (!byPayment[method]) byPayment[method] = { total: 0, banks: {} };
      byPayment[method].total += c.finalPaid;
      if (method === "信用卡") {
        const bank = (o.payment && o.payment.bank && o.payment.bank.trim()) || "未填寫銀行";
        byPayment[method].banks[bank] = (byPayment[method].banks[bank] || 0) + c.finalPaid;
      }
    });

    return { count: statsOrders.length, paidSum, discountSum, shippingSum, rewardSum, costSum, byPlatform, byCategory, byReward, byDay, byDayOrders, byMonth, byPayment };
  }, [statsOrders]);
...
function ListView({ orders, search, setSearch, expandedId, setExpandedId, onEdit, onDelete, onNew, hasAny }) {
  return (
      ...
      {orders.length === 0 ? (
        ...
        <ul className="order-list">
          {orders.map((o) => {
            ...
            const color = PLATFORM_COLORS[o.platform] || "var(--primary)";
                      ...
                      <div className="items-card">
                        ...
                        {o.images && o.images.length > 0 && (
                          <div className="image-grid view-only">
                            {o.images.map((img) => (
                              <a key={img.id} href={img.dataUrl} target="_blank" rel="noopener noreferrer" className="image-thumb view-only">
...
function FormView({ draft, setDraft, platforms, categories, shippingMethods, banks, cardNames, onCancel, onSave, isEditing }) {
  ...
  function addItemFromScan({ name, spec, quantity, price, image }) {
    setDraft((d) => {
      const firstEmpty = d.items.find((i) => !i.name.trim());
      if (firstEmpty) {
        return {
          ...d,
          items: d.items.map((i) => (i.id === firstEmpty.id ? { ...i, name, spec: spec || "", qty: quantity, price, image: image || i.image } : i)),
        };
      }
      return { ...d, items: [...d.items, { ...blankItem(), name, spec: spec || "", qty: quantity, price, image: image || "" }] };
    });
  }
  ...
  return (
    <div className="form-view">
      ...
      <section className="form-section">
        <h2><Tag size={16} strokeWidth={2.2} />折價券</h2>
      ...
      <section className="form-section">
        <h2><Truck size={16} strokeWidth={2.2} />運費與付款</h2>
        <div className="field-grid">
          <label className="field">
            <span>運費</span>
...
function ScreenshotScanner({ onRecognized }) {
  ...
  async function handleRecognize() {
    ...
    try {
      ...
      let thumbDataUrl;
      try {
        thumbDataUrl = result.thumbnailBBox
          ? await cropDataUrlByBBox(cropDataUrl, result.thumbnailBBox, 220, 0.6)
          : await resizeDataUrl(cropDataUrl, 220, 0.6);
      } catch (thumbErr) {
        thumbDataUrl = await resizeDataUrl(cropDataUrl, 220, 0.6);
      }
      onRecognized({ name: result.name, spec: result.spec, quantity: result.quantity, price: result.price, image: thumbDataUrl });
      const sourceNote = result.source ? `（來源：${result.source}）` : "";
      const specNote = result.spec ? `，規格：${result.spec}` : "";
      setMessage(`已加入：${result.name || "（未辨識出名稱）"} × ${result.quantity}，單價 ${fmt(result.price)}${specNote}${sourceNote}，請確認欄位是否正確。`);
  ...
  return (
    <div className="scanner-block">
      ...
      {!imageSrc ? (
        ...
        <>
          <p className="hint">在截圖上拖曳框選「一項商品」的範圍（縮圖、名稱、數量、價格都框進去即可，框大一點沒關係，AI 會自動找出正確的小縮圖），再按下方按鈕辨識；如果一張截圖有好幾項商品，請一次只框一項，完成後可以再框選下一項繼續新增。</p>
          <div
            className="scanner-canvas"
            ref={wrapRef}
            onMouseDown={onPointerDown}
...
function StatsView({ stats, statsMode, setStatsMode, periodLabel, shiftRef, refDate }) {
            ...
            <ul className="bar-list">
              {platformEntries.map(([name, val]) => (
                <li key={name}>
                  ...
                  <div className="bar-track"><div className="bar-fill" style={{ width: `${(val / maxPlatform) * 100}%`, background: PLATFORM_COLORS[name] || "var(--primary)" }} /></div>
          ...
          {paymentEntries.length === 0 ? <p className="hint">這段期間沒有資料。</p> : (
            <ul className="payment-list">
              {paymentEntries.map(([method, info]) => {
                const bankEntries = Object.entries(info.banks || {}).sort((a, b) => b[1] - a[1]);
                  ...
                  <li key={method} className="payment-item">
                    ...
                    {hasBanks && isOpen && (
                      <ul className="payment-sub-list">
                        {bankEntries.map(([bank, val]) => (
                          <li key={bank}><span>{bank}</span><span className="mono">{fmt(val)}</span></li>
                        ))}
                      </ul>
                    )}
                  </li>
                            ...
                            return (
                              <div key={tagKey} className="calendar-tag-wrap">
                                <button
                                  ...
                                  style={{ background: PLATFORM_COLORS[entry.platform] || "var(--primary)" }}
                  ...
                  <PieChart>
                    ...
                    >
                      {platformPieData.map((entry, i) => (
                        <Cell key={entry.name} fill={PLATFORM_COLORS[entry.name] || CATEGORY_PALETTE[i % CATEGORY_PALETTE.length]} stroke="var(--card)" strokeWidth={1} />
                      ))}
                    </Pie>
...
.image-thumb.view-only { cursor: zoom-in; }
...
.calendar-cell { min-height: 64px; border-radius: 8px; background: var(--bg); padding: 4px 3px; display: flex; flex-direction: column; gap: 2px; overflow: visible; position: relative; }
.calendar-cell.empty { background: transparent; }
.calendar-daynum { font-size: calc(11px * var(--font-scale, 1)); color: var(--text-soft); font-weight: 600; }
.calendar-tags { display: flex; flex-direction: column; gap: 2px; }
